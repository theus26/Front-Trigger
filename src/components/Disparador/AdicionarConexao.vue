...
import type { CreateInstance } from "@/interfaces/disparador";
...
const AdicionarInstancia = () => {
  ...
  const instance: CreateInstance = {
    instanceName: `account_${props.accountId}${instanceName.value}`.replace(
      / /g,
      "_"
    ),
  };
  postCriarInstance(instance).finally(() => {
    emit("getInstances");
    load.value = false;
  });
};
  ...
  <div class="tw-flex tw-h-full">
    ...
    <v-dialog v-model="dialog" max-width="400">
      <v-card rounded="lg">
        ...
        >
          <div class="text-h5 text-medium-emphasis ps-2">Adicionar conexão</div>
          <v-btn
            :icon="mdiClose"
            variant="text"
            @click="dialog = false"
