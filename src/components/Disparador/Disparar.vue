...
import { mdiReceiptTextSendOutline, mdiClose, mdiTrashCanOutline } from '@mdi/js'
import type { GetInstancesData, PostDataMessagesDisparar } from '@/interfaces/disparador'
...
const disparar = () => {
  load.value = true
  postDispararMensagens(props.instance.instanceName, numerosParaEnvio.value, messages.value).finally(() => {
    emit('getInstances');
    load.value = false
    notify()
  })
...
const tratarBase64 = (base64: string) => {
  ...
  if (base64.startsWith("data:")) {
    ...
    obj = {
      ...
      type: type.includes('audio') ? 'audio' : type.includes('image') ? 'image' : type.includes('application') ? 'document' : type.includes('video') ? 'video' : 'media',
    ...
    <v-dialog v-model="dialog" max-width="500">
      <v-card rounded="lg">
        ...
        <v-card-text class="tw-pt-1">
          <div class="tw-my-4 tw-font-normal tw-text-lg">1- Conteúdo do disparo.</div>
          <div v-for="(message, index) in messages" :key="index">
            <div class="tw-flex">
              <v-select @update:modelValue="(value) => setValueTypeMessage(value, index)" v-model="message.type" :items="typesOptions" item-title="label" item-value="value"
                label="Tipo do conteúdo" single-line></v-select>
              <div v-if="index != 0">
                <v-btn class="tw-bg-slate-300 tw-h-[55px] tw-ml-[4px] tw-f" size="small" @click="removerMessage(index)">
            ...
            <div v-if="message.type === 'image'">
              <div v-if="message.dataUrl">
                <img class="tw-h-full tw-w-full" :src="message.dataUrl" alt="Imagem selecionada" />
              </div>
            </div>
            <div v-if="message.type === 'video'" class="tw-w-full tw-h-[345px]">
              <div v-if="message.dataUrl" class="tw-w-full tw-h-full">
                <video class="tw-h-full tw-w-full" :src="message.dataUrl" controls :loop="true" :volume="1.0">
            ...
            <div v-if="message.type === 'document'">
              <div v-if="message.dataUrl">
                <iframe :src="message.dataUrl" allow="autoplay; fullscreen; picture-in-picture; clipboard-write" style="border: none;"></iframe>
              </div>
            </div>
            <div v-if="message.type === 'text'">
              <v-textarea rows="4" label="Mensagem" v-model="message.message"></v-textarea>
            </div>
            <div v-if="!message.dataUrl && message.type != 'text'" class="tw-mb-4">
              <input class="tw-hidden" ref="fileInputMessages" id="fuploadMessage" name="fuploadMessage" type="file"
                @change="(e) => onFileChange(e, index)" />
              <label for="fuploadMessage"
          ...
          <div>
            <v-btn class="ms-auto tw-bg-gray-300" text="Adicionar +" @click="adicionarObjetoMessage"></v-btn>
          </div>
        ...
        <template v-slot:actions>
          <v-btn :disabled="numerosParaEnvio.length == 0 || messages.length == 0" class="ms-auto tw-bg-gray-300"
